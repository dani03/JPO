    ...
    <body class="antialiased w-full h-full">
      <form id="questionsCard" class=" relative w-auto h-2/3 m-10 p-2 bg-gray-100 border border-indigo-400 rounded grid items-center justify-center" action="{{route('student.score')}}" method="post">
        ...
        <div  class=" {{$loop->first ? '' : 'hidden'}} bg-white m-4 border border-red-200 p-4 theQuestion">
          ...
          <div class="p-6 bg-white flex items-center  rounded-lg shadow-md">
            <div>
              <h1 class="text-lg font-bold text-gray-700 w-80 mb-2">{{$question->title}} </h1>     
              <input type="text" class="hidden" name="question[{{$question->title}}]" value="{{$question->point}}">
            </div>
          </div>          
          <div class="block bg-white justify-center" id="answers">
            @foreach($reponses->shuffle() as $reponse)
      ...
      <div class="flex justify-around">
        <button id="last" class="flex items-center px-2 py-2  text-gray-900  transform border rounded-lg dark:text-gray-500 dark:border-gray-200 ">precedent</button>
        <p class="flex items-center px-2 py-2"> <img class="w-16 h-full" src="{{asset('itech.png')}}" alt=""></p>
        <button id="next" class="flex items-center px-2 py-2  text-gray-900  transform border rounded-lg dark:text-gray-500 dark:border-gray-200 ">suivant</button>
      </div>
    </body>
